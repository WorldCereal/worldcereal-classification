...
# Set the root dir
OUTDIR_ROOT="/data/worldcereal_data/EXTRACTIONS/SENTINEL_1/"
...
# ============== TODO ==============
2019_CZE_LPIS_POLY_110 # is it possible that OOM is happenning?.. maybe restart with bigger overhead; 21 errors...
2020_CZE_LPIS_POLY_110 # 17 finished, 25 errors; will need to restart; start_failed for all jobs
...
for REF_ID in "${REF_IDS[@]}"; do
    ...
    # Run extraction
    "${PYTHONPATH}" "${EXTRACT_CMD}" PATCH_SENTINEL1 \
    "${OUTDIR}" "${REFDATA_FILE}" \
    --ref_id "${REF_ID}" \
