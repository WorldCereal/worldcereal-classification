"""Extract pre-processed S2 point data using OpenEO-GFMAP package.
for non-parametric approaches (NPA)"""
...
def generate_output_path_point_npa(
...
def create_job_dataframe_point_npa(
    ...
    for job in tqdm(split_jobs):
        ...
        # 6 months before and after the valid time
        start_date = (min_time - pd.Timedelta(days=6 * 31)).to_pydatetime()
        end_date = (max_time + pd.Timedelta(days=6 * 31)).to_pydatetime()
...
def create_job_point_npa(
    ...
    return cube.create_job(
        out_format="Parquet",
        title=f"NPA_Point_Extraction_{row.s2_tile}",
        job_options=job_options,
    )


def post_job_action_point_npa(
