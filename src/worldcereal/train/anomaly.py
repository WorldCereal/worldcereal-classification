...
def _rank_pct(metric: np.ndarray) -> np.ndarray:
    ...
    if metric.size == 0:
        return metric
    return pd.Series(metric).rank(pct=True, method="average").to_numpy(dtype=np.float32)
...
def run_pipeline(
    ...
    score_votes_suspect = (
        (flagged_df["rank_percentile"] >= suspect_thr).astype(int)
        + (flagged_df["S_rank"] >= suspect_thr).astype(int)
        + (flagged_df["S_z"] >= suspect_thr).astype(int)
    )
    score_votes_candidate = (
        (flagged_df["rank_percentile"] >= candidate_thr).astype(int)
